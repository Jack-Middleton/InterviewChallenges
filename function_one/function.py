...
def date_conversion(date):
    ...
    try:
        ...
        return "string for error handling, never gets printed"
...
def Inflation_factor(From_date, To_date, Effective_Dates, Percentage_Values):
    ...
    if to_date_days > Effective_Dates[-1] + 365:
    ...
    for i in range(len(Effective_Dates)):
        ...
        if Effective_Dates[i] < from_date_days and from_index == -1:
            from_index = i
        ...
        if Effective_Dates[i] < to_date_days:
    ...
    if from_index == to_index: # ie; the same interest rate
        ...
        cumulative_interest *= (1+Percentage_Values[from_index]/100) ** ((Effective_Dates[from_index + 1] - from_date_days) /365.25) 
        # essentially, works out the cumulative interest up to the next whole year
        for i in range(from_index + 1, to_index):
            cumulative_interest *= (1+Percentage_Values[i]/100) ** ((Effective_Dates[i + 1] - Effective_Dates[i]) /365.25) 
        ...
        cumulative_interest *= (1+Percentage_Values[to_index]/100) ** (to_date_days - Effective_Dates[to_index] /365.25) 
        # and calculates the final fraction of the years interest
        return cumulative_interest

while True:
    try:
        date = date_conversion(input("Enter a date in dd/mm/yyyy format: \n"))
        value = float(input("Enter a percentage value without the %: \n "))
    except:
        continue
    if not isinstance(date, str):
        ...
        print("Please enter date / percentage in valid format")
